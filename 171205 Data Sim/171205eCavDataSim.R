...
#set directory
setwd("~/Desktop/BMI206 Group Project/")
...
#import haps data
hapscase = read.table("hapgen_out_chr1.txt.cases.haps")
hapscont = read.table("hapgen_out_chr1.txt.controls.haps")
...
#saveLD
write.table(LDgwas, file="LDgwas",sep="\t",row.names = FALSE, col.names = FALSE,quote = FALSE);
write.table(LDeqtl, file="LDeqtl",sep="\t",row.names = FALSE, col.names = FALSE,quote = FALSE);
...
diag(LD) = -0.00001
maxIndex = which(LDgwas == max(abs(LDgwas),na.rm=T), arr.ind=T) 
gwasCausal = maxIndex[1];
eqtlCausal = maxIndex[1];
diag(LD) = 1;
causal = rep(0, nSNP);
...
Yeqtl = generate(Xeqtl,eqtlCausal);
causal[gwasCausal] = 0;
causal[eqtlCausal] = 1;
...
#savegwasF
write.table(cbind(SNPids, gwasF), file="gwasFscore", 
            sep="\t",row.names = FALSE, col.names = FALSE,quote = FALSE);
#saveeqtlF
write.table(cbind(SNPids, eqtlF), file="eqtlFscore", 
            sep="\t",row.names = FALSE, col.names = FALSE,quote = FALSE);
